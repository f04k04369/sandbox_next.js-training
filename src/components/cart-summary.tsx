...
const CartSummary = ({ restaurantId }: CartSummaryProps) => {
  ...
  if (cart === null) {
    return <div>カートが見つか見つかりません</div>;
  }

  const fee = 100;
  const service = 0;
  const delivery = 0;
  const subtotal = caluculateSubtotal(cart.cart_items);
        ...
        <Accordion type="single" collapsible defaultValue="item-1">
          <AccordionItem value="item-1">
            ...
            {cart.cart_items.map((cartItem) => (
              <AccordionContent className="flex items-center" key={cartItem.id}>
                <div className="flex items-center gap-4 flex-1">
                ...
                <label
                  htmlFor={`cart-quantity-${cartItem.id}`}
                  className="sr-only"
                >
                  数量
                </label>
                <select
                  ...
                  onChange={(e) =>
                    handleUpdateCartItem(e.target.value, cartItem.id)
                  }
